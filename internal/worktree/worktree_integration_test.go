...
// (never inside the user's repo), on a namespaced karya/<id> branch, and Remove
...
func TestAddRemoveIsolatesUnderKaryaRoot(t *testing.T) {
	...
	if out, _ := (execRunner{}).Output(repo, "git", "branch", "--list", "karya/t1"); !strings.Contains(out, "karya/t1") {
		t.Errorf("branch karya/t1 not created; got %q", out)
	}
	...
	if out, _ := (execRunner{}).Output(repo, "git", "branch", "--list", "karya/t1"); strings.TrimSpace(out) != "" {
		t.Errorf("branch karya/t1 survived Remove; got %q", out)
	}
}
