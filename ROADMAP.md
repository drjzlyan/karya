...
- ☐ Homebrew tap — cross-platform formula (macOS + Linux) generated by
  `scripts/update-formula.sh` from the release checksums and committed to `main`
  by the release workflow. **Wired; verified once the first tag ships.**
- ☐ Release automation — GoReleaser (`.goreleaser.yaml`) + `.github/workflows/release.yml`
  ...
  from commits. **Wired.** Cut `v0.1.0` first (end-to-end pipeline shakedown incl.
  self-update from a dev build), then tag `v1.0.0`.
